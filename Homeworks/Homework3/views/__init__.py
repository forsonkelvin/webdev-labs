...
def get_suggested_user_ids(current_user):
    # query the "following" table to get the list of unauthorized users:
    user_ids_tuples = (
        db.session
            .query(Following.id)
    ...
    auth_users_ids = get_authorized_user_ids(current_user)

    # convert to a list of ints:
    user_ids = set()
    for (id,) in user_ids_tuples:
        if id not in auth_users_ids:
            user_ids.add(id)
